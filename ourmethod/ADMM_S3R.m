...
function C2 = ADMM_S3R(Y, outliers, affine, alpha, Theta, gamma0, nu,  maxIter, Z, thr)
if (nargin < 2)
...
if (~outliers)
    ...
    if (~affine)
        ...
        while ( err1(i) > thr1 && i < maxIter )
            ...
            C2 = max(0,(abs(Z+Lambda2/mu2) - 1/mu2*(ones(N)./nu+ nu*gamma0 * (Theta)))) .* sign(Z+Lambda2/mu2);
            C2 = C2 - diag(diag(C2));
    ...
    if (~affine)
        ...
        while ( (err1(i) > thr1 || err2(i) > thr2) && i < maxIter )
            % updating Z
            Z = A * (mu1*P'*(Y+Lambda1/mu1)+mu2*(C1-Lambda2/mu2));      % Sparse Coding Q
         ...
         %C2                      = max(0,(abs(Z                     +Lambda2          /mu2) - 1/mu2* ones(N+D,N)))                                              .* sign(Z           +Lambda2         /mu2);        
            ...
            C2(1:N,:)           = max(0,(abs(Z(1:N,:)          +Lambda2(1:N,:)            /mu2) - 1/mu2*(ones(N,      N)./nu+ nu*gamma0 *(Theta)))) .* sign(Z(1:N,:)+Lambda2(1:N,:)/mu2);
            C2(1:N,:) = C2(1:N,:) - diag(diag(C2(1:N,:)));
            % updating Lagrange multipliers
            Lambda1 = Lambda1 + mu1 * (Y - P * Z); % Sparse E
            Lambda2 = Lambda2 + mu2 * (Z - C2);    % 
            % computing errors
            err1(i+1) = errorCoef(Z,C2);
        ...
        while ( (err1(i) > thr1 || err2(i) > thr2 || err3(i) > thr1) && i < maxIter )
         ...
         %C2                      = max(0,(abs(Z                     +Lambda2                     /mu2)  - 1/mu2* ones(N+D,N)))                              .* sign(Z           +Lambda2/mu2);
            C2(N+1:N+D,:)  = max(0,(abs(Z(N+1:N+D,:)+Lambda2(N+1:N+D,:) /mu2) - 1/mu2* ones(D, N) ))                                                   .* sign(Z(N+1:N+D,:)+Lambda2(N+1:N+D,:) /mu2);        
            C2(1:N,:)            = max(0,(abs(Z(1:N,:)           +Lambda2(1:N,:)          /mu2)  - 1/mu2*(ones(N    )./nu + nu*gamma0* (Theta)))) .* sign(Z(1:N,:)           +Lambda2(1:N,:)           /mu2);
